...
use solana_api_types::{
    Account, AccountMeta, Client, ClientError, ClientErrorKind, Instruction, Pubkey,
    RpcAccountInfoConfig, Signature, Transaction,
};
...
impl ApiClient {
    ...
    pub fn send_transaction(&self, transaction: Tx, _cfg: JsValue) -> Promise {
        let client = self.inner.clone();

        let fut = async move {
            // let cfg = cfg.into_serde()?;
            let r = client.send_transaction(transaction.as_ref()).await?;
...
impl solana_api_types::Signer for Keypair {
    fn try_pubkey(&self) -> Result<Pubkey, solana_api_types::signature::SignerError> {
        self.0.try_pubkey()
    }
    ...
    fn try_sign_message(
        &self,
        message: &[u8],
    ) -> Result<Signature, solana_api_types::signature::SignerError> {
...
mod tests {
    ...
    use solana_api_types::{
        RpcAccountInfoConfig, RpcSignatureStatusConfig, RpcSignaturesForAddressConfig,
        RpcTransactionConfig, TransactionEncoding, UiAccountEncoding,
    };
